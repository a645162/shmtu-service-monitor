...
/// Periodic poller that checks each registered instance's health endpoint.
pub struct Poller {
    pool: SqlitePool,
...
impl Poller {
    ...
    /// Run the poller loop. Polls all instances at their configured intervals.
    pub async fn run(self) {
        ...
        loop {
            ...
            let instances = match sqlite::list_all_instances(&self.pool).await {
                Ok(s) => s,
                Err(e) => {
                    error!("Failed to list instances for polling: {}", e);
                    continue;
                }
            };
            ...
            for inst in instances {
                ...
                tokio::spawn(async move {
                    if let Err(e) =
                        Self::poll_instance(&pool, &client, instance_id, &instance_name, &base_url)
                            .await
                    {
                        warn!("Poll failed for instance '{}' ({}): {}", instance_name, instance_id, e);
                    }
                });
            }

            // Periodic cleanup: keep last 10000 records per instance
    ...
    /// Poll a single instance and store the result.
    ...
    /// Attempt to fetch /api/status and /api/health from the instance.
